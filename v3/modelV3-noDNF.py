...
sys.stdout = Tee("model_v3_noDNF_output.txt")
...
def evaluateEnsemble(rfPred, xgbPred, mlpPred, yTrue, groupSeries, w_rf, w_xgb,
    ...
    race_rhos = []
    for _, idx in groupSeries.groupby(groupSeries).groups.items():
        actual_race  = yTrue.loc[idx]
        ranked_race  = ranked.loc[idx]
        if dnfMask is not None:
            keep = ~dnfMask.loc[idx]
            actual_race = actual_race[keep]
            ranked_race = ranked_race[keep]
        if len(actual_race) < 2:
            continue
        rho, _ = spearmanr(actual_race, ranked_race)
        if not np.isnan(rho):
            race_rhos.append(rho)

    if dnfMask is not None:
        no_dnf   = ~dnfMask
        yTrue_m  = yTrue[no_dnf]
        ranked_m = ranked[no_dnf]
    else:
        yTrue_m  = yTrue
        ranked_m = ranked
    ...
    return {
        ...
        'mae'       : mean_absolute_error(yTrue_m, ranked_m),
        'r2'        : r2_score(yTrue_m, ranked_m)
    }
...
dnfTestMask = pd.Series(
    df['is_dnf'].iloc[test_idx].fillna(0).astype(bool).values,
    index=xTest.index
)
...
#Metrics excluding DNF drivers
noDnfIdx  = ~dnfTestMask.values
mae_nodnf  = mean_absolute_error(yTest.values[noDnfIdx], yPred[noDnfIdx])
rmse_nodnf = np.sqrt(mean_squared_error(yTest.values[noDnfIdx], yPred[noDnfIdx]))
r2_nodnf   = r2_score(yTest.values[noDnfIdx], yPred[noDnfIdx])
 
print(f"\nFinal Ensemble Test Results "
      f"(RF={bestWRF:.0%} / XGB={bestWXGB:.0%} / MLP={bestWMLP:.0%})")
print(f"MAE:  {mae:.3f}  (excl. DNFs: {mae_nodnf:.3f})")
print(f"RMSE: {rmse:.3f}  (excl. DNFs: {rmse_nodnf:.3f})")
print(f"R²:   {r2:.3f}  (excl. DNFs: {r2_nodnf:.3f})")
 

results_test = metaTest.copy()
results_test['Actual']    = yTest.values
results_test['Predicted'] = yPred
results_test['is_dnf']    = df['is_dnf'].iloc[test_idx].fillna(0).astype(int).values
...
def weightedSpearman(actual, predicted):
    ...
    d2     = (actual - predicted) ** 2
    wrs    = 1 - (6 * np.sum(w * d2)) / (np.sum(w) * (n ** 2 - 1))
    return wrs
 
race_metrics = []
for race_id, grp in results_test.groupby('RaceID'):
    grp_fin   = grp[grp['is_dnf'] == 0]
    actual    = grp_fin['Actual'].values
    predicted = grp_fin['Predicted'].values
    if len(actual) < 2:
        continue
...
#Top-10-only per-race metrics — exclude DNF drivers too
top10_results = results_test[(results_test['Actual'] <= 10) & (results_test['is_dnf'] == 0)].copy()
...
ax_scatter.plot([1, 22], [1, 22], 'r--', linewidth=1.5)
...
textstr = (f'MAE  = {mae:.2f}  ({mae_nodnf:.2f} excl. DNF)\n'
           f'RMSE = {rmse:.2f}  ({rmse_nodnf:.2f} excl. DNF)\n'
           f'R²    = {r2:.3f}  ({r2_nodnf:.3f} excl. DNF)')
...
plt.savefig('rf_race_model_results2noDNF.png', dpi=150, bbox_inches='tight')
plt.show()
...
plt.savefig('graphs/rf_race_model_results_top10noDNF.png', dpi=150, bbox_inches='tight')
plt.show()

race_df.to_csv('csvs/rf_per_race_metrics2noDNF.csv', index=False)
top10_race_df.to_csv('csvs/rf_per_race_metrics_top10noDNF.csv', index=False)
weightDf.to_csv('csvs/ensemble_weight_search2noDNF.csv', index=False)
